<div class="relative">
    <div class={`flex items-center ${pad || 'p-1'}`}>
        <input type="text" 
            class="grow bg-transparent text-md placeholder:italic placeholder:text-sm placeholder:text-edge focus:outline-none text-inputtxt"
            {name}
            {placeholder}
            bind:value="{srch_str}"
            bind:this={input}
        ...
        <button class="ml-1 inline-flex items-center justify-center w-10 h-10 text-primary transition-colors duration-150 rounded-full focus:shadow-outline hover:bg-secondary"
        ...
        {:else}    
            <button class="inline-flex items-center justify-center w-10 h-10 text-primary transition-colors duration-150 rounded-full focus:shadow-outline hover:bg-secondary"
                on:click="{(event)=>{srch_str = ""; input.focus( ); is_open = false;}}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    <div class="{!is_open ? 'hidden' : 'absolute'} top-[51px] left-0 w-full md:max-h-[350px] border border-edge rounded overflow-y-auto">
    ...
    const dispatch = createEventDispatcher( ),
        ...
        onChange = async event => { // handle input change
            if( srch_str.trim( ).length >= Number( minchar ) ){
                ...
                ( debounce( e => {
                    dispatch( "query", srch_str ) // dispatch autocomplete search query
                    }, 200 ) )( )
