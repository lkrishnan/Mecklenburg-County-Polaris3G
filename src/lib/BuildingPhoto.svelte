<div class="w-full mb-2 px-4">
    {#if src}
        <div>
            ...
            {:else if failed}
                <calcite-icon icon="no-image" scale="m" class="w-36 h-36 stroke-secondary"></calcite-icon>
        
            {:else if loading}
                <calcite-icon icon="spinner" scale="m" class="animate-spin w-16 h-16 stroke-signify"></calcite-icon>
            {/if}    
        </div>
