...
<div class="relative">
    <div class="flex items-center {pad}">
        ...
        {#if go}
            <button class="ml-1 inline-flex items-center justify-center w-{btnsize} h-{btnsize} text-primero transition-colors duration-150 rounded-full focus:shadow-outline hover:bg-luz"
                on:click="{( )=>close( )}">
                <calcite-icon icon="search" scale="m" class="w-6 h-6 stroke-primero hover:stroke-segundo"></calcite-icon>
            </button>
        {/if}
        
        {#if spinner}
            <div class="inline-flex items-center justify-center w-{btnsize} h-{btnsize}">
                <calcite-icon icon="spinner" scale="m" class="animate-spin w-6 h-6 stroke-segundo"></calcite-icon>
            </div>
        {:else}    
            <button class="inline-flex items-center justify-center w-{btnsize} h-{btnsize} text-primero transition-colors duration-150 rounded-full focus:shadow-outline hover:bg-luz"
        ...
        <ul class="bg-surface shadow-lg rounded-b-md" bind:this={list} >
            {#if !nomatch}
                {#each results as result, i}
                    <li on:click="{()=>close(i)}" on:keydown="{()=>close(i)}" class="text-todo px-2 py-2 text-sm overflow-ellipsis overflow-hidden whitespace-nowrap hover:bg-luz hover:cursor-pointer last:rounded-b-md { i === arrowCounter ? ' bg-secondary' : '' }">
