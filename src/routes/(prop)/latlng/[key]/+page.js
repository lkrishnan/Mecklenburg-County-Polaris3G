...
export async function load( {fetch, params, route} ){
    ...
    if( srch_str.length === 0 )
        error( 404, { message: `Polaris 3G can't find anything. Enter valid Latitude and Longitude.` } );

    if( !validateLatLng( srch_str ) )
        error( 404, { message: `Polaris 3G can't find Latitude and Longitude: ${srch_str}. Enter valid Latitude and Longitude.` } );
