    ...
    try{
        ...
        if( pid || gisid || address ){
            const { assess_pool } = locals,
                filter = [ "address", "pid", "gisid" ]
                    .map( field => {
                        if( url.searchParams.get( field ) && field === "address" ){
                            ...
                            return `parcels.parcelid = '${pid}'`

                        }else if( url.searchParams.get( field ) && field === "gisid" ){
                            return `parcels.AssessorMap = '${gisid}'`
                ...
                sql = `SELECT parcels.parcelid as pid, parcels.AssessorMap as gisid, parcels.location_address as situs_address 
