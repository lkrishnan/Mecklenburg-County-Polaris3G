        ...
        if( pid || gisid || address ){
            const { cama_pool } = locals,
                ...
                sql = {
                    ...
                    info: `select id_pid as pid, id_common_pid as gisid, LTRIM(RTRIM(num_HouseNo)) as addrno, LTRIM(RTRIM(txt_stdir)) as prefix, 
                            LTRIM(RTRIM(txt_stname)) as stname, LTRIM(RTRIM(txt_STTYPE)) as sttype, LTRIM(RTRIM(txt_STSUFFIX)) as suffix, ISNULL(LTRIM(RTRIM(num_HouseUnit)),'') as unit, 
                            CDE_MUNIC_DESC AS juris
                            from tb_PUBLOCATION
                            where ${filter.join( " and " )}`
