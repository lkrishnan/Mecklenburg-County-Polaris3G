        ...
        if( name ){
            const sql = `SELECT regexp_replace(admkey, '\s*:.....', '') || 
                    CASE WHEN municipality = 'CHAR' THEN ', CHARLOTTE'
                        ...
                        ELSE ''
                    END as value, 'ROAD' as type, regexp_replace(admkey, '\s*:.....', '') || 
                    CASE WHEN municipality = 'CHAR' THEN ', CHARLOTTE'
                        WHEN municipality = 'CORN' THEN ', CORNELIUS'
                        WHEN municipality = 'DAVI' THEN ', DAVIDSON'
                        WHEN municipality = 'HUNT' THEN ', HUNTERSVILLE'
                        ...
                        WHEN municipality = 'MECK' THEN ', MECKLENBURG'
                        ELSE ''
                    END as road, preaddrnum as prefix, streetname as stname, streettype as sttype, addrnumsuf as suffix,	municipality as juris, countystcode as stcode
                    FROM streetfile_tb
                    WHERE admkey ~* $$${name}$$
                    LIMIT 5`,
