...
export async function load( {fetch, params, route, url} ){
    ...
    if( _last_hit.type !== srch_type.toUpperCase( ) || _last_hit.value !== srch_str.toUpperCase( ) ){ //request must have come from address bar of browser
        if( srch_str.length > 0 ){
            const validate_data = await fetch( getAPIURL( srch_type, srch_str ) + "&exact=1" ),
            ...
            if( data_rows.length > 0 ){
                ...
                throw error( 404, { message: `Polaris 3G can't find Owner with last name: ${srch_str}` } )
    
        }else
            throw error( 404, { message: `Polaris 3G can't find an anonymous Owner. Enter a valid Last Name` } )
