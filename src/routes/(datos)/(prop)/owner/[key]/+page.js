    ...
    if( _last_hit.type !== srch_type.toUpperCase( ) || _last_hit.value !== srch_str.toUpperCase( ) ){ //request must have come from address bar of browser
        if( validateOwnerName( srch_str ) ){
            ...
            if( data_rows.length > 0 ){
                ...
                throw error( 404, { message: `Polaris 3G can't find Owner with name: ${srch_str}` } )
    
        }else
            throw error( 404, { message: `Polaris 3G can't find an anonymous Owner. Enter a valid Owner Name` } )
