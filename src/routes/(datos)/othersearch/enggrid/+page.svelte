...
<script>
    ...
    onMount( async () => {
        const params = {
                table: "enggrid_py",
                columns: "map_sheet_no, ST_XMin(ST_Extent(shape)) as xmin, ST_YMin(ST_Extent(shape)) as ymin, ST_XMax(ST_Extent(shape)) as xmax, ST_YMax(ST_Extent(shape)) as ymax, map_sheet_no as value",
                group: "map_sheet_no",
                sort: "map_sheet_no",
